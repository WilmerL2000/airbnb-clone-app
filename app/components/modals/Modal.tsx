...
updated with the new value of `disabled`. */
  ...
  /* It checks if the disabled` prop is true, and if so, it returns without doing anything. Otherwise, it calls the
`onSubmit` function. The `onSubmit` function is passed as a dependency to `useCallback`, which means
that if it changes, a new version of `handleSubmit` will be created. The `disabled` prop is also a
dependency, which means that if it changes, the current version of `handleSubmit` will be updated
with the new value of `disabled`. */
